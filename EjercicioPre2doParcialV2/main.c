...
void imprimir_lista(ArrayList* listaEmpleados);
...
int generarArchivoSueldos(char* fileName,ArrayList* listaEmpleados)
{
    ...
    if(pFile!=NULL){
        ...
        empleados_getId(auxEmpleado,&id);
        empleados_getNombre(auxEmpleado,nombre);
        empleados_getHorasTrabajadas(auxEmpleado,horasTrabajadas);
        empleado_getSueldo(auxEmpleado,);
        }
    }
...
void imprimir_lista(ArrayList* listaEmpleados){
    ...
    for(i=0;i<al_len(listaEmpleados);i++){
        ...
        empleados_getNombre(auxEmpleado,nombre);
        empleados_getHorasTrabajadas(auxEmpleado,&horasTrabajadas);
        empleados_getSueldo(auxEmpleado,&sueldo);
        printf("%s,%d,%d,%d\n",nombre,horasTrabajadas,sueldo,id);
    }
}
